    ...
    <div className="bg-white">
      ...
      <div className="border-b border-gray-100" style={{ background: "#F8F8F4" }}>
        <div className="max-w-[800px] mx-auto px-6 py-3 flex items-center gap-2 text-[11px]" style={{ color: "#aaa", fontFamily: "var(--font-cinzel), serif" }}>
          <Link href="/es/blog" className="hover:text-[#C9A227] transition-colors">Blog</Link>
          <span>/</span>
      ...
      <div className="max-w-[800px] mx-auto px-6 pt-12 pb-8">
        ...
        <div className="flex items-center gap-3 text-[12px]" style={{ color: "#888", fontFamily: "var(--font-cinzel), serif" }}>
          ...
          <div>
            ...
            <time dateTime={post!.date}>{new Date(post!.date).toLocaleDateString("es-ES", { month: "long", day: "numeric", year: "numeric" })}</time>
          </div>
        </div>
      </div>
          ...
          <div className="max-w-[1200px] mx-auto px-6 py-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {featuredProducts.map((product) => (
                <Link key={product.slug} href={`/es/products/${product.slug}`} className="group block bg-white border border-gray-100 hover:shadow-md transition-shadow">
      ...
      {related.length > 0 && (
        <div className="max-w-[1200px] mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            {related.map((p) => (
              <Link key={p.slug} href={`/es/blog/${p.slug}`} className="group flex gap-5 items-start">
      ...
      <div className="max-w-[1200px] mx-auto px-6 pb-10">
        <Link href="/es/blog" className="text-[12px] font-bold tracking-wider hover:text-[#C9A227] transition-colors" style={{ color: "#888", fontFamily: "var(--font-cinzel), serif" }}>
          ← Volver al Blog
        </Link>
      </div>
    </div>
